  ...
  <div class="head_h" >
        ...
        >
          ...
          >
            <template slot-scope="scope" >
              <!--<router-link to="/">  id ='jjj'  @click.stop-->
              <el-button v-loading.fullscreen.lock="fullscreenLoading" @click="handleClick(scope.row)"  type="text" size="small" >打印</el-button>
            </template>
          </el-table-column>
          <!--<el-table-column-->
          <!--prop="tag"-->
    ...
    <div class="fast_contetn" >
      ...
      <el-dialog
        ...
        <div class="guanli-content2"  @click="loseContent">
          ...
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px"  class="demo-ruleForm">
            ...
            <el-form-item label="默认显示" prop="moRen">
              <!--v-model="ruleForm.moRen"-->
              <div contenteditable="true" id="textDiv" @click.stop="listContent($event) " >
                <el-tag
                  :key="tag"
              ...
              <div class="listContent" v-show="listShow">
              <ul class="listUl" >
                <li class="listLi" v-bind:class="{ active:index1== isActive }" @click.stop="setTag(list,index1)" v-for="(list,index1) in  gather" :key="index1">{{list.appellation}}</li>
    ...
    <side-bar v-if="isShow" :closeSidebar="handleClickSide" :closeSide="handleClicka" :tabledata1="row"></side-bar>
    <loan-side v-if="isShows"  :closeSide="handleClicka" :tabledata2="row1"></loan-side>

    <!--:closeSidebar="handleClickSide1"-->
  </div>
</template>

<script>
  import SideBar from './commen/sideBar'
  import LoanSide from './commen/LoanSide'
  export default {
    data(){
      return{
        // tableData: [{
    ...
    methods: {
      ...
      setTag(list,index1){
        ...
        if(this.currentIndex && this.currentIndex !== index1){
              alert(1)
        this.dynamicTags.push(list.appellation)
          console.log(list.appellation);

      }else if(this.currentIndex == index1){
        console.log(this.currentIndex);
        alert(2)
          this.isActive = false
          this.dynamicTags.map((a)=>{
            console.log(a);
            if(list.appellation==a){
              this.dynamicTags.splice(list.appellation,1)
            }
          })
      ...
      handleClickSide(row){
        // console.log(row);
          ...
          if(row.numbers == 98453212){
            alert(1)
            this.isShow = true
            this.row=row
          }else if(row.numbers == 54985123){
            this.isShows = true
            this.row=row
          }
...
<style scoped>
  ...
  .guanli-content2{ overflow: auto; width: 100%;height:350px; border-top: 1px solid #e6e6e6; border-bottom: 1px solid #e6e6e6; padding-top:20px; }
  .guanli-content2::-webkit-scrollbar {display: none; }
  ...
  .listUl{ border: 1px solid #e6e6e6; padding: 0;margin-top: 0px; height: 250px;overflow: auto;}
  .listUl::-webkit-scrollbar {display: none; }
  .listLi{list-style: none;margin-bottom: 2px;}
  .listLi:hover{ background-color: #eff9fa;}
  /*点击样式*/
  .listUl .active{background-color: #fafafa; font-weight: bold; }
  /*.listContent{ position: absolute; top: 200px;left: 0;z-index: 2000;}*/
